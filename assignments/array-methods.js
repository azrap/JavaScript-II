...
allCaps=runners.map((ele) => {
    return ele["first_name"].toUpperCase();
});
console.log(allCaps); 
...
// Problem 1
// We want to identify who our top donors are. Provide a list of all runners who have donated at least $200.

let topDonors=[];
topDonors=runners.filter(ele => ele["donation"]>200);
console.log(topDonors);

// Problem 2
// We want to get a list of all the companies that our runners come from.
let companies=[]
runners.forEach((ele)=> {
 
    companies.push(ele["company_name"]);
})
console.log(companies);


// Problem 3
// We need to send emails to our runners. Please provide a list of all email addresses for the runners

let emails=[]
runners.forEach((ele)=> {
 
    companies.push(ele["email"]);
})
console.log(emails);